...
class Msg91PendingReqIdStore:
    def __init__(self, *, redis_client: Any = None, ttl_seconds: float = 300.0, key_prefix: str = "m360") -> None:
    ...
    def discard_identifier(self, identifier: str) -> None:
        ...
        if self._redis is not None:
            try:
                self._redis.delete(self.pending_key(ident))
            except Exception as exc:
                logger.warning("MSG91 pending redis delete failed: %s", exc)
            return
        with self._lock:
    ...
    def record(self, identifier: str, request_id: str) -> None:
        ...
        if self._redis is not None:
            try:
                self._redis.setex(self.pending_key(ident), self._ttl, rid)
            except Exception as exc:
                logger.warning("MSG91 pending redis set failed: %s", exc)
            return
        with self._lock:
    ...
    def take(self, identifier: str) -> Optional[str]:
        ...
        if self._redis is not None:
            try:
                ...
                logger.warning("MSG91 pending redis take failed: %s", exc)
                return None
        with self._lock:
