...
class PhoneAuthService:
    ...
    def retry_otp(
        ...
        data = self._otp_gateway.retry_otp(self._widget_id, req_id, retry_channel)
    ...
    def verify_otp(self, phone_number: str, req_id: str, otp: str) -> Dict[str, Any]:
        ...
        data = self._otp_gateway.verify_otp(self._widget_id, req_id, otp)
    ...
    def _extract_req_id(data: Dict[str, Any]) -> Optional[str]:
        ...
        if otp_type == "success":
            return raw_message
        return (data or {}).get("reqId") or (data or {}).get("request_id")
