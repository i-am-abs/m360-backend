...
class MasjidController:
    def __init__(self, masjidSearchService: MasjidSearchService, userMasjidService: UserMasjidService,
                 userRepository: UserRepository, locationBasedFeatureFlagService: LocationBasedFeatureFlagAdapter,
    ...
    def searchNearby(self, latitude: float, longitude: float, radius: int, maxResultCount: int) -> Dict[str, Any]:
        if not self.locationBasedFeatureFlagService.isFeatureEnabledAtLocation("MASJID_SEARCH_FEATURE", latitude,
                                                                               longitude):
            raise ApiException(
    ...
    def searchByName(self, query: str, limit: int, radius: Optional[int]) -> Dict[str, Any]:
        return self.masjidSearchService.search_by_name(query, limit, radius)

    def searchByCity(self, city: str, maxResultCount: int, radiusMeters: int) -> Dict[str, Any]:
    ...
    def getMasjidDetails(self, placeId: str, currentUserId: str) -> Dict[str, Any]:
        ...
        if lat is not None and lng is not None:
            hasDonations = self.locationBasedFeatureFlagService.isFeatureEnabledAtLocation("DONATIONS_FEATURE",
                                                                                           float(lat), float(lng))
            hasAnnouncements = self.locationBasedFeatureFlagService.isFeatureEnabledAtLocation("ANNOUNCEMENTS_FEATURE",
                                                                                               float(lat), float(lng))
        else:
            hasDonations = False
...
@router.get("/masjids/nearby", summary="Search nearby masjids")
def get_masjid_nearby(latitude: float, longitude: float, radius: int = 1_000, max_result_count: int = 10,
...
@router.get("/masjids/search", summary="Search masjid by name")
@router.get("/search", summary="Search masjid (short)")
def search_masjid_by_name(q: str, max_result_count: int = 10, maxResultCount: Optional[int] = Query(None, ge=1),
                          radius_meters: Optional[int] = Query(None, ge=500, le=50_000),
                          radiusMeters: Optional[int] = Query(None, ge=500, le=50_000, ),
                          controller: MasjidController = Depends(get_masjid_controller),
...
@router.get("/masjids/by-city", summary="Search masjids by city")
def get_masjid_by_city(city: str, max_result_count: int = 20,
                       radius_meters: Optional[int] = Query(None, ge=500, le=50_000),
                       controller: MasjidController = Depends(get_masjid_controller),
                       settings: Settings = Depends(get_settings)):
...
@router.get("/masjids/place", summary="Get place by ID")
def get_masjid_place(place_id: str = Query(..., description="Google Place ID"),
                     controller: MasjidController = Depends(get_masjid_controller)):
    return success_response(controller.getPlaceById(place_id))


@router.get("/masjids/status", summary="Masjid module status")
...
@router.get("/masjids/{place_id}/details", summary="Get masjid full details")
...
@router.get("/users/me/masjids", summary="List my favourite masjids")
...
@router.post("/users/me/masjids/{place_id}", summary="Add masjid to favourites")
...
@router.delete("/users/me/masjids/{place_id}", summary="Remove from favourites")
